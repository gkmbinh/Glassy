...
def client_secrets
  Google::APIClient::ClientSecrets.load('./client_secrets.json').to_authorization
end
...
def refesh_auth_tooken(credentials)
   ...
   puts "REFESH TOOKEN"
   client.authorization = client_secrets
   client.authorization.refresh_token = credentials[:refresh_token]
